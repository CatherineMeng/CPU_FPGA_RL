...
void loadIn(blockvec In[],  hls::stream<a0blockvec> &a0_buf_fifo,hls::stream<blockvec> &Inrows,const int LL,int ind){
	...
	#pragma HLS bind_storage variable=Inrows_local type=RAM_2P impl=bram
...
void fw_l1(hls::stream<blockvec> &Inrows, hls::stream<w1blockvec> &a1_buf_fifo, w1blockvec bias, w1blockvec Wcols[], hls::stream<blockvec> &Crows, hls::stream<bsbit> &actder_fifo,const int LL,const int LN) {
	...
	#pragma HLS bind_storage variable=z1_buf_local type=RAM_2P impl=bram
    ...
    #pragma HLS aggregate variable=tempA
     #pragma HLS aggregate variable=tempB
		for(int i = 0; i < BSIZE/P; i++) {
			...
			// #pragma HLS dependence variable=outc inter false
				for(int ii = 0; ii < P; ii++) {
					#pragma HLS UNROLL
					for(int jj = 0; jj < T; jj++) {
						#pragma HLS UNROLL
						// float atimesb=tempA.a[i*P+ii]*tempB.a[j*T+jj];
						// float biasc=bias.a[j*T+jj];
						...
						// if (k==0) outc=atimesb;
						// else if (k==LL-1) outc += (atimesb + biasc);
						// else outc += atimesb;
						// z1_buf_local[i][j][ii][jj]=outc;
					}
				}
			}
		}
...
// void test_target(blockvec *r, actvec *action, float gamma, bsbit *done, hls::stream<blockvec> &outpipe1, hls::stream<blockvec> &outpipe3, hls::stream<blockvec> &outpipe6,  hls::stream<w3blockvec> &delt2_buf_fifo, int ind,
// 	hls::stream<ap_axiu<32,0,0,0>> &pn_out){
// 	#pragma HLS aggregate variable=outpipe1
// 	#pragma HLS aggregate variable=outpipe3
// 	#pragma HLS aggregate variable=outpipe6
...
// 	w3blockvec d2tmp [BSIZE];
// 	for(int i = 0; i < L3; i++) { //this factor consistent with a1_buf_fifo partition
// 		blockvec tempC=outpipe1.read();
// 		// blockvec tempD=outpipe3.read();
// 		blockvec tmpobj;
// 		for (int j=0;j<BSIZE;j++){
// 			if (i==action_local.a[j]) //only happens BSIZE times
// 			{
// 				float actdertmp=(tempC.a[j]>0)? 1:0; //relu derivative
// 				float oneb=1-done_local.a[j]; //cast fixed point to float
// 				tmpobj.a[j]=2*(r_local.a[j]+oneb*gamma*argmax_tq.a[j]-tempC.a[j])*actdertmp; 
// 				// Qtransfer[j]=tempC.a[j];
// 				TDtransfer[j]=tmpobj.a[j]/2;
// 			}
// 			else
// 				{tmpobj.a[j]=0;}
// 			// tmpobj.a[j]=tempC.a[j]+argmax_tq.a[j];
// 			d2tmp[j].a[i]=tmpobj.a[j];
			
// 		}
// 		outpipe6.write(tmpobj);
// 	}
...
void objctv(blockvec *r, actvec *action, float gamma, bsbit *done, hls::stream<blockvec> &Qrows,hls::stream<blockvec> &Qtrows, hls::stream<blockvec> &outs,
	...
	#pragma HLS aggregate variable=done
	#pragma HLS aggregate variable=delt2_buf_fifo
	#pragma HLS aggregate variable=outs
	#pragma HLS aggregate variable=pn_out
	...
	// Get Q vals, calc obj
	w3blockvec d2tmp [BSIZE]; //for writing to delt2 buf, aggregate L3, size = BSIZE
	float Qtransfer[BSIZE];
	float TDtransfer[BSIZE];
	#pragma HLS bind_storage variable=d2tmp type=RAM_1P impl=bram
	#pragma HLS bind_storage variable=Qtransfer type=RAM_1P impl=bram
	#pragma HLS bind_storage variable=TDtransfer type=RAM_1P impl=bram
...
void test_sub_backmm2(hls::stream<blockvec> &Inrows, hls::stream<bsbit> &actder_fifo, hls::stream<w1blockvec> &delt1_buf_fifo){
...
hls::stream<ap_axiu<32,0,0,0>> &pn_out/*Replay args*/){
	...
	#pragma HLS INTERFACE s_axilite port=w1bram_out
	#pragma HLS INTERFACE s_axilite port=w2bram_out
	...
	#pragma HLS INTERFACE s_axilite port=wsync
	...
	#pragma HLS aggregate variable=w1bram_out
	#pragma HLS bind_storage variable=w1bram type=RAM_2P impl=bram
	...
	#pragma HLS bind_storage variable=w1bram_t type=RAM_1P impl=uram 
	...
	#pragma HLS bind_storage variable=bias1_t type=RAM_1P impl=bram
	#pragma HLS bind_storage variable=bias2_t type=RAM_1P impl=bram
...
//	Init on-chip memory
	...
	#pragma HLS bind_storage variable=delt1_buf_fifo type=fifo impl=URAM 
	...
	#pragma HLS bind_storage variable=a1_buf_fifo type=fifo impl=URAM 
	...
	#pragma HLS bind_storage variable=w2bram_copy type=RAM_1P impl=bram
	...
	for(int ind=0; ind<BATCHS; ind++){
		...
		// test_target(r, acts, gamma, done,outpipe1,outpipe3,outpipe6, delt2_buf_fifo,ind,pn_out);
