...
// void Top_tree(int insert_signal,int insert_ind,int upd, float pn[],int ind_o[]){
	...
	int insert_ind;
	static int par_m[BS][D-1]; //memoinization array for parent nodes, useful in later update process
	int ind_curr;
	...
	while(1){
		...
		if(!q_samp_out.full()){
			...
			samp_main_loop:for (int i=0;i<BS;i++){
				#pragma DATAFLOW
				...
				Sib_Iterator_l3(init_fan4.TLev3,l2_out_pipe,q_samp_out,i);
		...
		if (!q_insert.empty()){
			// ============================priority insertion============================
			ind_curr++;
			init_fan4.TLev2[ind_curr/K] += 1; //parent0
			init_fan4.TLev1[(ind_curr/K)/K] += 1; //parent1
			init_fan4.TLev0 += 1; //parent2
