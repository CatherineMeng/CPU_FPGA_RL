...
int main(int argc, char **argv)
{
  ...
  printf("flags set\n");
    ...
    krnl_read1.setArg(2, N_actor);
    krnl_read2.setArg(0, inpn_buf_learn);
    krnl_read2.setArg(2, N_learner);
    krnl_write.setArg(1, out_buf);
    krnl_write.setArg(2, N_learner);
   ...
   // Copy input data to device global memory
    ...
    int insert_size= N_actor;//can be changed at runtime
    int update_size= N_learner;//can be changed at runtime
    queue<float> update_q ;
    queue<float> insert_q ;
    queue<int> sample_q ;
    i_init=0;
    // mimics the RL outer-most loop
    for (int j=0; j<2; j++){
        // producer threads: actor, learner
        for (k = 0; k < insert_size; k++) {
            insert_q.push(insert_size+j); 
        }
        for (k = 0; k < update_size; k++) {
            update_q.push(update_size-j);
        }
        // consumer threads: rmm
        for (k = 0; k < insert_size; k++) {
            pn_in_act[k] = insert_q.front(); 
            // std::cout << "insert_q" << k<<":"<<float(insert_q.front()) <<std::endl;
            insert_q.pop(); 
        }
        for (k = 0; k < update_size; k++) {
            pn_in_learn[k]=update_q.front();
            std::cout << "update_q" << k<<":"<<float(update_q.front()) <<std::endl;
            update_q.pop();
        }
        ...
        krnl_write.setArg(2, N_learner);
       // Copy input data to device global memory
        std::cout << "Copying data from insertion & update queues..." << std::endl;
        q.enqueueMigrateMemObjects({inpn_buf_act}, 0 /*0 means from host*/);
        q.enqueueMigrateMemObjects({inpn_buf_learn}, 0 /*0 means from host*/);
        ...
        // Copy Result from Device Global Memory to Host Local Memory
        std::cout << "Getting Sampling Results..." << std::endl;
        q.enqueueMigrateMemObjects({out_buf}, CL_MIGRATE_MEM_OBJECT_HOST);
        q.finish();
        //producer thread: RMM
        for (k = 0; k < update_size; k++) {
            sample_q.push(ind_o_out[k]);
        }
        //consumer thread: Data Storage
        for (k = 0; k < update_size; k++) {
            sample_q.pop();
        }
        // OPENCL HOST CODE AREA END
