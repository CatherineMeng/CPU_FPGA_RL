...
void fw_l1(hls::stream<blockvec> &Inrows, w1blockvec a1_buf[BSIZE],float bias[], w1blockvec Wcols[], hls::stream<blockvec> &Crows, bsbit actder[L2],const int LL,const int LN) {
	...
	//add bias, find actder, write activatpon out to stream
...
void sub_backmm2(blockvec Inrows[], w3blockvec Wcols[], bsbit actder[L2],w1blockvec delt1_buf[BSIZE], const int LL,const int LN){
	#pragma HLS aggregate variable=Inrows
...
void learners_top(blockvec *S, blockvec *Snt, w1blockvec w1bram_out[L1],w3blockvec w2bram_out[L2],int wsync){
	...
	#pragma HLS INTERFACE m_axi port=S bundle=gmem4 offset=slave
	#pragma HLS INTERFACE m_axi port=Snt bundle=gmem4 offset=slave
...
//	Init on-chip memory
	...
	if (wsync==0 or wsync==1){
		...
		for (int i=0; i<L2;i++){
			bias1[i]=bias1_list[i];
		}
		for (int i=0; i<L3;i++){
			bias2[i]=bias2_list[i];
		}
	...
	fw_bw(S,w1bram,w2bram,bias1, bias2,wa1_global,wa2_global);
