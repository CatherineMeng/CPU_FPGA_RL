...
// 	i = i -1;
// 	//calculating the start addr of next layer sibling iteration
// 	data_out.start=(pipstart+i)*K;
// 	//calculating the new x to be found in the next layer
// 	data_out.newx=x-prev_pref;
// 	ind_newxout.write(data_out);
// 	parent[index_learners][1]=pipstart+i;
// }
...
void Top_tree(int insert_signal,int *insert_ind,float *init_priority, int update_signal, hls::stream<ap_axiu<32,0,0,0>> &pn_in,int sample_signal,int *ind_o){
	#pragma HLS INTERFACE m_axi port=insert_ind bundle=gmem1 offset=slave
	#pragma HLS INTERFACE m_axi port=init_priority bundle=gmem1 offset=slave
...
//=====================populate tree for testbench purposes=============
	...
	// 	printf("TLev3 index %d priority %f\n",i,float(init_fan4.TLev3[i]));
	// 	printf("TLev2 index %d priority %f\n",i/K,float(init_fan4.TLev2[i/K])); //parent0
	// 	printf("TLev1 index %d priority %f\n",(i/K)/K,float(init_fan4.TLev1[(i/K)/K])); //parent1
	...
	// 	// fixed_insrt p_ini
	// 	for (int i=0;i<0+insert_batch;i++){
	// 		p_ini[i]=init_priority[i];
	// 		insert_ind_local[i]+=init_priority[i]; 
	// 	}
	// 	for (int i=0;i<0+insert_batch;i++){
	// 		int ind_curr=insert_ind_local[i];
	// 		init_fan4.TLev3[ind_curr]=p_ini[i];
	// 		init_fan4.TLev2[ind_curr/K]+=p_ini[i]; //parent0
	// 		init_fan4.TLev1[(ind_curr/K)/K]+=p_ini[i]; //parent1
	// 		init_fan4.TLev0+=p_ini[i]; //parent2
	// 	}

	// #ifndef __SYNTHESIS__
	// printf("Tree content after insertion\n");
	// for (int i=insert_ind;i<insert_ind+insert_batch;i++){
	// 	printf("Actor at index %d: \n",i);
	// 	printf("TLev3 index %d priority %f\n",i,float(init_fan4.TLev3[i]));
	// 	printf("TLev2 index %d priority %f\n",i/K,float(init_fan4.TLev2[i/K])); //parent0
	// 	printf("TLev1 index %d priority %f\n",(i/K)/K,float(init_fan4.TLev1[(i/K)/K])); //parent1
	...
	if (sample_signal==1){
		...
		fixed_root rngs [N_learner]={1.0, 4.9, 6.0, 5.0, 2.0, 17.0, 2.0, 6.1}; //should return 1,3,51,3,17,25,2,5
		// int ind_o[N_learner]; //on the top port
		
		samp_main_loop:for (int i=0;i<N_learner;i++){
			#pragma DATAFLOW
			// #ifndef __SYNTHESIS__
			// printf("Entered Dataflow for number %f\n",float(rngs[i]));
			// #endif 
