...
void Top_tree(int insert_signal,int insert_ind,int upd, float pn[],int ind_o[]){
	#pragma HLS INTERFACE m_axi port=insert_signal bundle=gmem0 offset=slave
	#pragma HLS INTERFACE m_axi port=insert_ind bundle=gmem1 offset=slave
	#pragma HLS INTERFACE m_axi port=upd bundle=gmem2 offset=slave
	...
	fixed_root rngs [N_learner]={1.0, 4.0, 76.0, 5.0, 25.0, 37.0, 2.0, 6.1}; //should return 1,3,51,3,17,25,2,5
	...
	samp_main_loop:for (int i=0;i<N_learner;i++){
		...
		Sib_Iterator_l3(init_fan4.TLev3,l2_out_pipe,ind_o,i);
		#ifndef __SYNTHESIS__
		printf("l3\n");
		#endif
